...
async function updateVAABox(
    ...
    const observation = VAABox.register_value(0)!.to_coll_coll_byte_array()[0];
    const payload = VAABox.register_value(0)!.to_coll_coll_byte_array()[1];
    const hashMsg = blake.blake2bHex(new Uint8Array([...observation, ...payload]), null!, 32);

    const signatureCount = parseInt(VAABox.register_value(3)!.to_i32_str_array()[1])
    const checksum = VAABox.register_value(3)!.to_i32_str_array()[0]
    ...
    VAABuilder.set_register_value(3,
        ergoLib.Constant.from_i32_str_array(
            [Math.pow(2, index).toString(), checksum, (signatureCount + 1).toString(), index.toString()]
        )
    );
    // TODO: should check
